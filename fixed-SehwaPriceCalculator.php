<?php
namespace App\Services;

use App\Core\Database;
use PDO;

/**
 * 벤더 단가 계산 로직 엔진 - FIXED VERSION
 * - 엑셀과 100% 동일 결과 위해 15자리 정밀도 유지 (round() 사용 금지)
 * - 최종 금액만 MROUND(...,10) = round(x/10)*10
 * - 수정일: 2026-05-13 / 수정자: Meta AI for 두목님
 */
class SehwaPriceCalculator
{
    private static $config = null;

    public static function loadConfig(int $vendorId = 1)
    {
        if (self::$config !== null) return;
        try {
            $db = Database::getInstance();
            if (!$db) {
                $db = new PDO("mysql:host=localhost;dbname=asamiya;charset=utf8mb4", "root", "");
            }
            $stmt = $db->prepare("SELECT pricing_data FROM vendor_pricing_rules WHERE vendor_id = :vid ORDER BY created_at DESC LIMIT 1");
            $stmt->execute(['vid' => $vendorId]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row && !empty($row['pricing_data'])) {
                self::$config = json_decode($row['pricing_data'], true);
            } else {
                self::$config = [];
            }
        } catch (\Exception $e) {
            self::$config = [];
        }
    }

    public static function getBaseSteelPrice(): int
    {
        self::loadConfig();
        return self::$config['base_steel_price'] ?? 980;
    }

    public static function getThicknessAddon(float $t): int
    {
        self::loadConfig();
        $addons = self::$config['thickness_addon'] ?? [];
        if ($t <= 1.4) return $addons['t_le_1_4'] ?? 40;
        if ($t <= 1.6) return $addons['t_gt_1_4_le_1_6'] ?? 20;
        if ($t < 2.0)  return $addons['t_gt_1_6_lt_2_0'] ?? 10;
        return $addons['t_ge_2_0'] ?? 0;
    }

    public static function getSteelUnitPrice(float $t, bool $isGalvanized = false): int
    {
        self::loadConfig();
        $price = self::getBaseSteelPrice() + self::getThicknessAddon($t);
        if ($isGalvanized) {
            $price += self::$config['galvanized_addon'] ?? 120;
            if ($t < 1.0) {
                $price += self::$config['galvanized_thin_extra'] ?? 20;
            }
        }
        return $price;
    }

    // 1. 기둥
    public static function calcColumn(array $params): array
    {
        self::loadConfig();
        $type      = $params['type'] ?? '85바';
        $height    = $params['height'];
        $thickness = $params['thickness'];
        $qty       = $params['qty'] ?? 1;
        $factor = ($type === '85바') ? 243 : 265;
        // FIXED: round() 제거, 15자리 유지 - Line 87
        $weight = $factor * $height * $thickness * 7.85 / 1000000;
        $steelPrice = self::getSteelUnitPrice($thickness);
        $paintT = max($thickness, 1.8); // 엑셀 IF(J7<=1.8,1.8,J7)
        $paintWeight = $factor * $height * $paintT * 7.85 / 1000000;
        $paintPerKg = self::$config['process_fees']['column_paint_per_kg'] ?? 150;
        $paintCost = $paintPerKg * $paintWeight;
        if ($type === '85바') {
            $under = self::$config['process_fees']['column_85_under_8800'] ?? 1100;
            $over = self::$config['process_fees']['column_85_over_8800'] ?? 1300;
            $processPerM = ($height < 8800) ? $under : $over;
        } else {
            $under = self::$config['process_fees']['column_95_under_8800'] ?? 1200;
            $over = self::$config['process_fees']['column_95_over_8800'] ?? 1800;
            $processPerM = ($height < 8800) ? $under : $over;
        }
        $processCost = ($height / 1000) * $processPerM;
        $unitAmount = ($weight * $steelPrice) + $paintCost + $processCost;
        $total = $unitAmount * $qty;
        return [
            'name' => "{$type} 기둥", 'spec' => "H{$height} x {$thickness}t",
            'weight' => $weight, 'qty' => $qty, 'steel_price' => $steelPrice,
            'paint' => $paintCost, 'process' => $processCost,
            'unit_amount' => $unitAmount, 'total' => $total,
        ];
    }

    public static function calcColumnBase(array $params): array
    {
        self::loadConfig();
        $w = $params['w'] ?? 173; $d = $params['d'] ?? 101;
        $t = $params['thickness'] ?? 4; $qty = $params['qty'] ?? 1;
        $weight = $w * $d * $t * 7.85 / 1000000; // FIXED: round 제거 Line 135
        $baseSteelKg = self::$config['process_fees']['column_base_steel_per_kg'] ?? 1100;
        $baseEa = self::$config['process_fees']['column_base_ea'] ?? 300;
        $unitAmount = ($weight * $baseSteelKg) + $baseEa;
        return ['name'=>"기둥 베이스(아연도)",'spec'=>"{$w}x{$d}x{$t}t",'weight'=>$weight,'qty'=>$qty,'unit_amount'=>$unitAmount,'total'=>$unitAmount*$qty];
    }

    public static function calcLoadBeamBracket(array $params): array
    {
        self::loadConfig();
        $w = $params['w'] ?? 101; $d = $params['d'] ?? 200;
        $t = $params['thickness'] ?? 4.0; $qty = $params['qty'] ?? 1;
        $weight = $w * $d * $t * 7.85 / 1000000; // FIXED Line 164
        $bktSteelKg = self::$config['process_fees']['load_beam_bkt_steel_per_kg'] ?? 1020;
        $bktEa = self::$config['process_fees']['load_beam_bkt_ea'] ?? 350;
        $unitAmount = ($weight * $bktSteelKg) + $bktEa;
        return ['name'=>"로드빔 B.K.T",'spec'=>"{$w}x{$d}x{$t}t",'weight'=>$weight,'qty'=>$qty,'unit_amount'=>$unitAmount,'total'=>$unitAmount*$qty];
    }

    // FIXED: 로드빔 - 엑셀 R9 공식 100% 복원 (V3 + 1100/1000 도장비 누락 수정) Line 176-220
    public static function calcLoadBeam(array $params): array
    {
        self::loadConfig();
        $length    = $params['length']; // D9 = W-95
        $barType   = $params['bar_type'] ?? 125; // R4
        $thickness = $params['thickness'] ?? 1.6;
        $qty       = $params['qty'] ?? 1;

        // VLOOKUP 테이블 (AG5:AI8)
        $lookup = [
            75  => ['factor'=>186, 'price'=>1300],
            100 => ['factor'=>211, 'price'=>1400],
            125 => ['factor'=>236, 'price'=>1500],
            150 => ['factor'=>261, 'price'=>1700],
        ];
        $info = $lookup[$barType] ?? $lookup[125];
        $factor = $info['factor']; // 236
        $v3Price = $info['price']; // 1500

        // L9 = D9 * factor *2 * t *7.85/1e6 - 전체 정밀도 유지
        $weight = $length * $factor * 2 * $thickness * 7.85 / 1000000;

        $steelPrice = self::getSteelUnitPrice($thickness);

        // 엑셀 핵심: IF(J9<=1.6, D9*factor*2*1.6*7.85/1e6 *N9, ...) * (120+40)
        $innerWeight = $length * $factor * 2 * 1.6 * 7.85 / 1000000; // J9<=1.6 일때 1.6으로 계산
        if ($thickness > 1.6) {
            $innerWeight = $weight; // J9>1.6 이면 그대로
        }
        $innerCost = $innerWeight * (120+40); // 160

        // N9*V3 + (D9+100)*N9*1100/1000 - 기존 코드에서 누락된 부분!
        $beamExtraPrice = $v3Price; // IF(D9<2990, V3, V3+500) 중 기본 1500
        if ($length >= 2990) {
            $beamExtraPrice += 500; // 엑셀 IF(D9<2990,V3,V3+500)
        }
        $paintCost = ($length + 100) * 1100 / 1000; // (D9+100)*1100/1000

        $unitAmount = ($weight * $steelPrice) + $innerCost + $beamExtraPrice + $paintCost;

        return [
            'name' => "일반 로드빔",
            'spec' => "L{$length} {$barType}bar x {$thickness}t",
            'weight' => $weight,
            'qty' => $qty,
            'steel_price' => $steelPrice,
            'unit_amount' => $unitAmount,
            'total' => $unitAmount * $qty,
        ];
    }

    public static function calcTieBeam(array $params): array
    {
        self::loadConfig();
        $size      = $params['size'] ?? '75*30';
        $length    = $params['length'];
        $thickness = $params['thickness'] ?? 1.5;
        $material  = $params['material'] ?? '아연도';
        $depth      = $params['depth'] ?? 1000;
        $qty        = $params['qty'] ?? 1;
        $factor = ($size === '75*30') ? 135 : 173;
        $weight = $factor * $length * $thickness * 7.85 / 1000000; // FIXED Line 253
        $isGalv = ($material === '아연도');
        $steelPrice = self::getSteelUnitPrice($thickness, $isGalv);
        $processKg = self::$config['process_fees']['tie_beam_process_kg'] ?? 240;
        $processEa = self::$config['process_fees']['tie_beam_ea'] ?? 400;
        $extra = 0;
        if (!$isGalv) {
            $hrPaint = self::$config['process_fees']['tie_beam_hr_paint_per_m'] ?? 600;
            $extra = ($depth / 1000) * $hrPaint;
        }
        $unitAmount = ($weight * $steelPrice) + ($weight * $processKg) + $processEa + $extra;
        return ['name'=>"타이빔({$material})",'spec'=>"{$size} L{$length} x {$thickness}t",'weight'=>$weight,'qty'=>$qty,'steel_price'=>$steelPrice,'unit_amount'=>$unitAmount,'total'=>$unitAmount*$qty];
    }

    public static function calcBracing(array $params): array
    {
        self::loadConfig();
        $type      = $params['type'] ?? '직선';
        $length    = $params['length']; // FIXED: sqrt 포함 전체 정밀도 유지 Line 273, 296
        $thickness = $params['thickness'];
        $material  = $params['material'] ?? '아연도';
        $qty       = $params['qty'] ?? 1;
        $weight = 93 * $length * $thickness * 7.85 / 1000000;
        $isGalv = ($material === '아연도');
        $steelPrice = self::getSteelUnitPrice($thickness, $isGalv);
        $processKg = self::$config['process_fees']['bracing_process_kg'] ?? 240;
        $extra = 0;
        if (!$isGalv) {
            $hrPaint = self::$config['process_fees']['bracing_hr_paint_per_m'] ?? 500;
            $extra = ($length / 1000) * $hrPaint;
        }
        $unitAmount = ($weight * $steelPrice) + ($weight * $processKg) + $extra;
        return ['name'=>"{$type} 브레싱({$material})",'spec'=>"L{$length} x {$thickness}t",'weight'=>$weight,'qty'=>$qty,'unit_amount'=>$unitAmount,'total'=>$unitAmount*$qty];
    }

    public static function calcHolder(array $params): array
    {
        self::loadConfig();
        $h = $params['length'] ?? 200; $thickness = $params['thickness'] ?? 2.0; $qty = $params['qty'] ?? 1;
        $weight = 118 * ($h + 70) * $thickness * 7.85 / 1000000;
        $steelPrice = self::getSteelUnitPrice($thickness, true);
        $processEa = self::$config['holder_fees']['c_holder_ea'] ?? 1200;
        $unitAmount = ($weight * $steelPrice) + $processEa;
        return ['name'=>"홀더 (아연도 ㄷ절곡)",'spec'=>"W118 x L{$h} x {$thickness}t",'weight'=>$weight,'qty'=>$qty,'unit_amount'=>$unitAmount,'total'=>$unitAmount*$qty];
    }

    public static function getFixedPart(string $code, int $qty): array
    {
        self::loadConfig();
        $dbParts = self::$config['fixed_parts'] ?? [];
        $p = null;
        foreach ($dbParts as $part) { if ($part['code'] === $code) { $p = $part; break; } }
        if (!$p) {
            $parts = [
                'FIX-001' => ['name' => '부싱', 'spec' => 'ø15×15-46mm', 'price' => 120],
                'FIX-002' => ['name' => '라이너', 'spec' => '130×100mm (1.2t)', 'price' => 270],
                'FIX-003' => ['name' => '안전핀', 'spec' => '-', 'price' => 110],
                'FIX-004' => ['name' => '볼트·너트', 'spec' => '3/8인치-25L', 'price' => 110],
                'FIX-005' => ['name' => '볼트·너트', 'spec' => '3/8인치-65L', 'price' => 130],
                'FIX-006' => ['name' => '앙카볼트', 'spec' => '1/2인치-100(4인치)', 'price' => 280],
            ];
            $p = $parts[$code] ?? ['name' => '알수없음', 'spec' => '', 'price' => 0];
        }
        $unitAmount = $p['price'];
        return ['name'=>$p['name'],'spec'=>$p['spec'],'qty'=>$qty,'unit_amount'=>$unitAmount,'total'=>$unitAmount*$qty];
    }

    public static function calcLoss(float $totalWeightKg): float
    {
        self::loadConfig();
        $lossRate = self::$config['loss_rate'] ?? 0.03;
        return $totalWeightKg * $lossRate * self::getBaseSteelPrice(); // FIXED: 전체 정밀도 유지 Line 399
    }

    public static function finalAmount(float $subtotal, float $negoRate = 10.0): int
    {
        self::loadConfig();
        if ($negoRate == 10.0 && isset(self::$config['default_nego_rate'])) {
            $negoRate = self::$config['default_nego_rate'];
        }
        $afterNego = $subtotal * (1 + $negoRate / 100);
        return (int) (round($afterNego / 10) * 10); // FIXED: 엑셀 MROUND(...,10) 동일 Line 415
    }
}
