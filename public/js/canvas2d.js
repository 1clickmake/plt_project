...
window.resetCanvas = function() {
    ...
    // 입력폼 초기화
    const fieldsToClear = ['pallet-w', 'pallet-d', 'pallet-h', 'pallet-weight', 'forklift-lift-height', 'forklift-ast', 'rack-levels', 'rack-bays', 'rack-height'];
    ...
    if (isExtendingRack && extendingRackIndex >= 0 && extendingRackIndex < racks.length) {
        ...
        if (snapMergeTarget) {
            ...
            if (!isValid && newTotalBays > extendInitialBays) {
                // 일반 연결이 장애물에 닿아 들어갈 수 없으면, 드래그 확장 한도를 직전 유효 칸수로 락
...
window.spawnRack = function(isDouble, bays) {
    if (currentScale <= 0) {
        alert("도면을 먼저 그려주세요!");
        return;
    }
    const defaultBays = (bays && bays > 0) ? bays : 3;
    const isHoriz = true;
    const dir = 1;
    const independent = 1;
    const connected = Math.max(0, defaultBays - 1);
    ...
    // 길이 계산
    const totalLenMm = 85 + (defaultBays * beamLength);
    ...
    // 즉시 드래그(이동) 모드로 진입하여 마우스 커서에 스냅되도록 처리
    window.isMovingRack = true;
    window.currentRackPreview = newRack;
    
    // 마우스의 중심점을 잡기 위해 Offset 계산 (랙의 가운데를 잡도록)
    window.rackDragOffsetX = -(totalLenMm * currentScale / 2);
    window.rackDragOffsetY = -(rackDepth * (isDouble ? 2 : 1) * currentScale / 2);
